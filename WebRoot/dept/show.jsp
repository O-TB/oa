	...
	<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
		<tr>
			<td align="left" valign="top" id="addinfo">
				<a href="#" target="mainFrame" onFocus="this.blur()" class="add">部门信息</a>
			</td>
		</tr>
		<tr>
			...
			<td align="left" valign="top">
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
					<tr>
						...
						<th align="center" valign="middle" class="borderright">部门描述</th>
						<c:if test="${role == '管理员'  }">
							<th align="center" valign="middle">操作</th>
						</c:if>
					</tr>
					<c:forEach var="data" items="${all }" varStatus="i">
						<tr onMouseOut="this.style.backgroundColor='#ffffff'"
							onMouseOver="this.style.backgroundColor='#edf5ff'">
							<td align="center" valign="middle" class="borderright borderbottom">${i.count}</td>
							<td align="center" valign="middle" class="borderright borderbottom">${data.name }</td>
							<td align="center" valign="middle" class="borderright borderbottom">${data.manager }</td>
							<%--  <c:if test="${role eq '主任' }">
			<td align="center" valign="middle" class="borderbottom">
	           <a href="<%=path %>/dept/findById.do?id=${data.id}" target="mainFrame" onFocus="this.blur()" class="add">编辑</a>
	        </td>
			</c:if> --%>
							<c:if test="${role eq '管理员' }">
								<td align="center" valign="middle" class="borderbottom">
									<a href="<%=path %>/dept/findById.do?id=${data.id}" target="mainFrame"
										onFocus="this.blur()" class="add">编辑</a>
									<span class="gray">&nbsp;|&nbsp;</span>
									<a href="<%=path %>/dept/del.do?id=${data.id}" target="mainFrame" onFocus="this.blur()"
