	...
	<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
		...
		<tr>
			<td align="left" valign="top" id="addinfo">
				<a href="#" target="mainFrame" onFocus="this.blur()" class="add">请假信息</a>
			</td>
		</tr>
		<tr>
			...
			<td align="left" valign="top">
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
					<tr>
						...
						<th align="center" valign="middle" class="borderright">请假原因</th>
					...
					<c:forEach var="data" items="${all }">
						<tr onMouseOut="this.style.backgroundColor='#ffffff'"
							...
							<td align="center" valign="middle" class="borderright borderbottom">${data.offDate }</td>
							<td align="center" valign="middle" class="borderright borderbottom">${data.offTime }</td>
							<td align="center" valign="middle" class="borderright borderbottom">${data.reason }</td>
							<td align="center" valign="middle" class="borderright borderbottom">${data.publish }</td>

							<td align="center" valign="middle" class="borderbottom">
								<c:if test="${role eq '普通员工' }">
									<a href="<%=path %>/offs/findById.do?id=${data.id}" target="mainFrame"
										onFocus="this.blur()" class="add">修改</a>
									<span class="gray">&nbsp;|&nbsp;</span>
									<a href="<%=path %>/offsApply/add.do?id=${data.id}" target="mainFrame"
										onFocus="this.blur()" class="add">销假</a>
									<span class="gray">&nbsp;|&nbsp;</span>
								</c:if>
								<a href="<%=path %>/offs/del.do?id=${data.id}" target="mainFrame" onFocus="this.blur()"
									class="add" onClick="return fun()">删除</a>
								<c:if test="${role eq '主任' }">
									<span class="gray">&nbsp;|&nbsp;</span>
									<a href="<%=path %>/offs/yes.do?id=${data.id}" target="mainFrame" onFocus="this.blur()"
										class="add">通过</a>
									<span class="gray">&nbsp;|&nbsp;</span>
									<a href="<%=path %>/offs/no.do?id=${data.id}" target="mainFrame" onFocus="this.blur()"
										class="add">驳回</a>
								</c:if>
